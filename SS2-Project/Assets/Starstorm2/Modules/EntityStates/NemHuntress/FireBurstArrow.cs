...
{
    public class FireBurstArrow : BaseSkillState
    {
        public float charge;
        ...
        public virtual void FireProjectile()
        {
            if (!hasFired)
            {
                ...
                if (isAuthority)
                {
                    ...
                    if (charge >= 0.142)
                    {
                        ProjectileManager.instance.FireProjectile(projectilePrefab, aimRay.origin, Util.QuaternionSafeLookRotation(new Vector3(aimRay.direction.x, aimRay.direction.y + 5, aimRay.direction.z)), gameObject, damage, 0f, RollCrit(), DamageColorIndex.Default, null, projectileSpeed);
                        if (characterBody.skillLocator.primary.stock != 1) characterBody.skillLocator.primary.stock--;
                    }
                }
            }
        }
