    ...
    public class FireGrenadeLauncher : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            if (isAuthority)
            {
                ...
                {
                    SS2Log.Error("FireGrenadeLauncher: NemToolbotController not found.");
                    outer.SetNextStateToMain();
                    return;
                }
                if (!projectilePrefab)
                {
                    SS2Log.Error("NemToolbot FireGrenadeLauncher: projectilePrefab is null.");
                    outer.SetNextStateToMain();
                    return;
                }
                if (!controller.TryConsumeAmmo(NemToolbotController.WeaponType.GrenadeLauncher))
                {
                    outer.SetNextStateToMain();
                    return;
                }
