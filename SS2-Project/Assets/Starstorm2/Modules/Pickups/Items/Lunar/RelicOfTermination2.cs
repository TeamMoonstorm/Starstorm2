...
namespace Moonstorm.Starstorm2.Items
{
    public sealed class RelicOfTermination2 : ItemBase
    {
        ...
        public sealed class Behavior : BaseItemBodyBehavior, IOnKilledOtherServerReceiver
        {
            ...
            [ItemDefAssociation]
            private static ItemDef GetItemDef() => SS2Content.Items.RelicOfTermination2;
            ...
            {
                if (!NetworkServer.active || illegalMarks.Contains(obj.bodyIndex))
                {
                    return;
                }
                foreach (var player in PlayerCharacterMasterController.instances)
                {
                    ...
                    {
                        var holderToken = player.GetComponent<TerminationHolderToken>();
                        if (!holderToken)
                        {
                            holderToken = player.gameObject.AddComponent<TerminationHolderToken>();
                        }
                        //Debug.Log("spawning - has buff");
                        if (obj.inventory && !holderToken.target)
                        {
                            int count = player.body.GetItemCount(SS2Content.Items.RelicOfTermination2);
                            if(count == 0)
                            {
