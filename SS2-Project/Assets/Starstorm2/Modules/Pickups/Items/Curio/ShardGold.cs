...
namespace SS2.Items
{
	public sealed class ShardGold : SS2Item
	{
		public override SS2AssetRequest AssetRequest => SS2Assets.LoadAssetAsync<ItemDef>("ShardGold", SS2Bundle.Items);
        ...
        private void SpawnGoldShard(On.RoR2.HalcyoniteShrineInteractable.orig_DropRewards orig, HalcyoniteShrineInteractable self)
        {
			...
			if(NetworkServer.active)
            {
				...
				if (participatingPlayerCount > 0 && self.rewardDropTable)
				{
					int num = participatingPlayerCount;
					float angle = 360f / (float)num;
					Vector3 vector = Quaternion.AngleAxis((float)UnityEngine.Random.Range(0, 360), Vector3.up) * (Vector3.up * 40f + Vector3.forward * 5f);
					Quaternion rotation = Quaternion.AngleAxis(angle, Vector3.up);
					Vector3 position = self.transform.position + self.rewardOffset;
					int i = 0;
					while (i < num)
					{
